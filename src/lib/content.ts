 ...
 *
 * Fallback: if DATABASE_URL is missing, the DB query fails, or row id=1
 * doesn't exist, the function logs a one-line warning and returns the
 * imported seed.json so that local dev and `next build` work with no DB.
 */

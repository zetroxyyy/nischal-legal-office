 ...
 * for 300 s and can be invalidated on-demand with updateTag("content").
 */
 ...
 * Returns exact same SiteContent shape as getContent() and falls back to seedJson.
 */
export async function getUncachedContent(): Promise<SiteContent> {
  const dbUrl = process.env.DATABASE_URL ?? process.env.POSTGRES_URL;
  if (!dbUrl) {
    console.warn("[getUncachedContent] No DATABASE_URL — using local seed.json fallback.");
    return localSeed;
  }
  ...
  try {
    ...
    if (!rows || rows.length === 0) {
      console.warn(
        "[getUncachedContent] site_content row id=1 not found — using local seed.json fallback."
      );
      return localSeed;
    }
    return rows[0].data as SiteContent;
    ...
    console.warn(
      "[getUncachedContent] DB query failed — using local seed.json fallback.",
      String(err)
    );
    return localSeed;
  }
